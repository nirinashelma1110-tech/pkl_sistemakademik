...
$pdf->Cell(42, 7, 'Nama Mahasiswa', 1, 0, 'C');
$pdf->Cell(30, 7, 'Kontak', 1, 0, 'C');
$pdf->Cell(60, 7, 'Email', 1, 0, 'C');
$pdf->Cell(25, 7, 'Jenis Kelamin', 1, 1, 'C');
...
if ($rv > 0) {
    ...
    while ($data = mysqli_fetch_array($query_ambil_mahasiswa)) {
        $pdf->Cell(10, 7, $no++, 1, 0, 'C');
        $pdf->Cell(20, 7, $data['nim'], 1, 0, 'C');
        $pdf->Cell(42, 7, $data['nama'], 1, 0, 'L');
        $pdf->Cell(30, 7, $data['kontak'], 1, 0, 'C');
        $pdf->Cell(60, 7, $data['email'], 1, 0, 'C');
        $pdf->Cell(25, 7, ($data['kelamin'] == 'L') ? 'Laki-laki' : "Perempuan", 1, 1, 'C');
    }
}
$pdf->Output();
?>
