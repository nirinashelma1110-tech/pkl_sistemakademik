...
if (isset($_POST['btn_impor'])){ // cek apakah button sudah diklik 
    ...
    $kode_kelas = mysqli_real_escape_string($con, $_POST['kode_kelas']); // ambil data kode kelasnya
    ...
    // tampung data dari excel
       ...
       $cek_duplikat = mysqli_query($con, "SELECT * FROM tbl_detail_kelas WHERE kode_kelas = '$kode_kelas' AND nim='$nim'");
       $rv = mysqli_num_rows($cek_duplikat);

       if ($rv == 0) {
         $query_insert = mysqli_query($con, "INSERT INTO tbl_detail_kelas (kode_kelas, nim) VALUES ('$kode_kelas','$nim')")or die(mysqli_error($con));
    ...
    alert("Data Berhasil Diimport");
     window.location.href = "../detail_kelas_matkul/?user=' . $kode_kelas . '";
    </script>';
